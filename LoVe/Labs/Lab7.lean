...
Prove the following two theorems by structural induction on `t`. -/
...
Declare `BTree` an instance of `AddMonoid` using `graft` as the
addition operator. 

The `nsmul` fields are implementation details that you don't need to 
worry about.-/
...
Explain why `BTree` with `graft` as addition cannot be declared an
...
Prove the following theorem illustrating why `BTree` with `graft` as
...
Prove that the multiset of nodes does not change when mirroring a tree.
...
Prove that the finite set of nodes does not change when mirroring a
...
Show that this does not hold for the list of nodes by providing a
