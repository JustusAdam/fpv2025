...
Complete the following big-step semantics, based on the informal
...
Prove the following inversion rules, as we did in the lecture for the
WHILE language. -/
...
Complete the translation below of a deterministic program to a GCL
...
In the definition of `gcl_of` above, `skip` is translated to
...
Prove the following program equivalences. -/
...
Program equivalence can be used to replace subprograms
