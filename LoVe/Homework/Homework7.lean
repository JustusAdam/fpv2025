...
structure ListGraph (V : Type) where
  (edges : List (V × V))
...
structure PredGraph (V : Type) where
  (adj : V → V → Prop)
  (symm : Symmetric adj)
...
structure GraphAutomorphism (α : Type) (G : PredGraph α) where
  (f : α → α)
...
axiom GraphAutomorphism.inv_mul_cancel :
...
@[instance] def AutomorphismGroup_C₅ :
  ...
  inv_mul_cancel := GraphAutomorphism.inv_mul_cancel,
}
...
| step {G mid next endV vs} :
  (adj: G.adj mid next) → (rest: IsPath G next endV (next::vs))
    → IsPath G mid endV (mid :: next :: vs)
| fin {G endV} : IsPath G endV endV [endV]
...
| empty {vs} : ListDisj [] vs
| cons {x xs vs} : (x ∉ vs) → ListDisj (x::xs) vs
-- Fill this in!
...
def IsBiconnected {α : Type} (G : PredGraph α) : Prop :=
  ∀ (x y : α), x ≠ y → ∃ (p₁ p₂ : List α), IsPath G x y (x :: p₁ ++ [y]) ∧ IsPath G x y (x :: p₂ ++ [y]) ∧ ListDisj p₁ p₂
...
@[autogradedProof 2]
theorem K₄_fin_is_biconnected : IsBiconnected K₄_fin := by
  unfold IsBiconnected
  intros x y h_eq
  have ⟨ a, b, h_neqs ⟩ := exist_distinct_fin4_of_neq x y h_eq
  have ⟨ x_neq_a, a_neq_y, x_neq_b, b_neq_y, a_neq_b ⟩ := h_neqs
  apply Exists.intro
  swap ; exact [a]
  apply Exists.intro
  swap ; exact [b]
  constructor
  repeat (constructor ; unfold K₄_fin ; simp ; assumption)
  constructor
  constructor
  repeat (constructor ; unfold K₄_fin ; simp ; assumption)
  constructor
